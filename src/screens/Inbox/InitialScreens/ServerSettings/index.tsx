...
import {ServerSettingsProps} from './types';
import {ServerSettingsSchema} from './utils';

export type InboxNavProps = CompositeScreenProps<
  BottomTabScreenProps<BottomNavParamList, 'Inbox'>,
  NativeStackScreenProps<EmailStackParamList, 'ServerSetting'>
>;

export const ServerSettings = (props: InboxNavProps) => {
  ...
  const InitialValues: ServerSettingsProps = {
    userName: '',
    password: '',
    server: '',
  };
    ...
    <Formik<ServerSettingsProps>
      ...
      {({handleSubmit}) => {
        return (
          <Container noSpacing>
            <Header
              label={t('inboxPage:serverSettingsTitle')}
            ...
            <Stack style={styles().initialTop}>
              <KeyboardAwareScrollView>
                <StackItem childrenGap={16} spacing={16} spaceBelow={16}>
                  <FormikTextField
                    name="userName"
                    label={t('inboxPage:userName')}
                    placeholder={t('inboxPage:userNamePlaceholder')}
                  ...
                  <FormikTextField
                    name="server"
                    label={t('inboxPage:serverUrl')}
                    placeholder={t('inboxPage:serverPlaceholder')}
                    keyboardType="email-address"
                  />
                  <Stack
