...
export const ServerSettingsSchema = () => {
  const schema = yup.object().shape({
    userName: yup.string().required(Translate('inboxPage:userNameError')),
    password: yup
      .string()
      .matches(passwordRegexValidator, Translate('inboxPage:passwordError1'))
      .matches(passwordEmojiValidator, Translate('inboxPage:userNameError'))
      .required(Translate('inboxPage:passwordError')),
    server: yup.string().required(Translate('inboxPage:serverError')),
  });
